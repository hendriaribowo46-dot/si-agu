...
export default function App() {
  ...
  const [nilaiData, setNilaiData] = useState({}); // Stores grades
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        // Cek token lingkungan khusus (untuk preview environment)
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
          await signInWithCustomToken(auth, __initial_auth_token);
        } 
        // KITA HAPUS signInAnonymously di sini agar user harus login manual
        // jika tidak ada custom token.
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // 1. Fetch Identitas & Guru
    const unsubSettings = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'settings'), (snapshot) => {
    ...
    // 2. Fetch Kelas
    const unsubKelas = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'kelas'), (snapshot) => {
      const list = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      setKelas(list.sort((a, b) => a.nama.localeCompare(b.nama)));
    }, (err) => console.error("Kelas error", err));

    // 3. Fetch Siswa
    const unsubSiswa = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'siswa'), (snapshot) => {
      const list = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
      setSiswa(list);
    }, (err) => console.error("Siswa error", err));

    // 4. Fetch Nilai
    const unsubNilai = onSnapshot(collection(db, 'artifacts', appId, 'users', user.uid, 'nilai'), (snapshot) => {
      const data = {};
      snapshot.docs.forEach(doc => {
  ...
  // --- HELPERS ---
  const saveToFirestore = async (collectionName, docId, data, merge = true) => {
    if (!user) return;
    try {
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, collectionName, docId), data, { merge });
    } catch (e) {
  ...
  const currentContextKey = `${tahunPelajaran.replace('/', '-')}_${semester}`;

  // --- RENDERERS ---

  if (loading) return <div className="flex h-screen items-center justify-center bg-gray-100 text-gray-600">Memuat Aplikasi...</div>;
    ...
    <div className="flex h-screen bg-gray-100 font-sans text-gray-800 overflow-hidden">
      ...
      <aside className="w-64 bg-slate-800 text-white flex flex-col print:hidden shadow-xl z-20">
        <div className="p-4 border-b border-slate-700 flex items-center gap-2">
          ...
          <h1 className="font-bold text-lg">e-SPN Sistem Pengolahan Nilai</h1>
        </div>
        
        <div className="p-4 bg-slate-900">
          <label className="text-xs text-slate-400">Tahun Pelajaran</label>
          <select 
            value={tahunPelajaran} 
            onChange={(e) => setTahunPelajaran(e.target.value)}
            className="w-full bg-slate-800 border-none text-sm rounded mt-1 p-1 focus:ring-1 focus:ring-blue-500"
          >
          ...
          <label className="text-xs text-slate-400 mt-2 block">Semester</label>
          <select 
            value={semester} 
      ...
      <main className="flex-1 overflow-y-auto bg-gray-50 p-6 print:p-0 print:bg-white">
        ...
        {activeMenu === 'kelas' && (
          <DataKelas 
            ...
            onDelete={async (id) => {
              ...
              await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'kelas', id));
            }}
          />
        )}
        {activeMenu === 'siswa' && (
          <DataSiswa 
            ...
            onDelete={async (id) => {
              ...
              await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'siswa', id));
            }}
          />
        )}
        {activeMenu === 'penilaian' && (
          <Penilaian 
            ...
            nilaiData={nilaiData}
            onSaveNilai={(key, data) => saveToFirestore('nilai', key, data)}
          />
        )}
        {activeMenu === 'analisis' && (
          <Analisis 
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><School size={20}/> Identitas Sekolah</h3>
      ...
      {/* Data Guru */}
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2"><Users size={20}/> Data Guru & Mata Pelajaran</h3>
...
function DataKelas({ kelas, onSave, onDelete }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
       <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">Manajemen Kelas</h2>
          ...
          <div className="md:col-span-2 bg-white p-4 rounded shadow border">
             <table className="w-full text-sm text-left">
                ...
                <tbody>
                  ...
                  {kelas.length === 0 && <tr><td colSpan="3" className="p-4 text-center text-gray-400">Belum ada kelas</td></tr>}
                </tbody>
             </table>
          </div>
       </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
       <div className="flex justify-between items-center border-b pb-2">
         <h2 className="text-2xl font-bold text-gray-800">Data Peserta Didik</h2>
...
// 4. PENILAIAN (GRADING)
function Penilaian({ siswa, kelas, guru, contextKey, nilaiData, onSaveNilai }) {
  ...
  const docId = selectedKelas && selectedMapel ? `${contextKey}_${selectedKelas}_${selectedMapel}` : null;
...
// 5. ANALISIS
function Analisis({ siswa, kelas, guru, contextKey, nilaiData, onSaveNilai }) {
  const [selectedKelas, setSelectedKelas] = useState('');
  const [selectedMapel, setSelectedMapel] = useState('');
  ...
  const handleRemidiChange = (studentId, val) => {
    ...
    onSaveNilai(docId, { ...currentData, scores: newScores });
  };
                   ...
                   <td className="p-2 border">
                     {!isTuntas && (
                       <input 
                         ...
                         onBlur={() => onSaveNilai(docId, currentData)} // Save on blur
                       />
                     )}
                   </td>
                   <td className="p-2 border">
                      <input 
                         ...
                         onBlur={() => onSaveNilai(docId, currentData)}
                      />
                   </td>
                 </tr>
               )
...
// 6. LAPORAN (REPORTING)
function Laporan({ identitas, siswa, kelas, guru, contextKey, nilaiData, tp, sem }) {
  ...
  const docId = config.kelasId && config.mapel ? `${contextKey}_${config.kelasId}_${config.mapel}` : null;
  const currentData = nilaiData[docId] || null;
